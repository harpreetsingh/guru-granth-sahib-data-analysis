...
- Clear User-Agent: `ggs-text-analysis/<version>`
...
normalized form. This means normalization changes DO change `line_uid`s.
...
Polysemy registry (`lexicon/polysemy.yaml`):
  Maps surface forms to multiple `entity_id`s with context hints
...
For each window, collect set of unique `entity_id`s.
