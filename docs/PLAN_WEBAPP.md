PLAN_WEBAPP.md
Guru Granth Sahib Analytical Web Platform
0. Philosophy
...
No scraping or heavy compute in production.
...
Static hosting (Vercel / similar)

Data:

Prebuilt JSON bundle

Optionally loaded via CDN
...
Total divine mentions

Nirgun vs Sagun distribution

Register density overview
...
Each claim links to line-level evidence.

2.3 Structural Explorer

Sanskritic vs Perso-Arabic density heatmap

Co-occurrence graph

Author gravitation ranking
...
2.4 Nirgun vs Sagun Panel

Distribution across authors

Timeline progression

Line examples per category

Explicit disclaimer:
Descriptive tagging, not doctrinal claim.
...
Paginated concordance

CSV export

3. Performance Strategy

Load only aggregates first

Lazy-load concordance pages
...
Webapp is successful if:

Users can verify every chart

Every count links to Gurmukhi

No opaque black-box logic
...
Research-grade core architecture (PLAN.md v3)

Webapp-ready frontend strategy (PLAN_WEBAPP.md)

Next best step:

Start implementing Phase 0 + corpus validator first, because everything downstream depends on it.

give me a readme to init the repository
...
Create a deterministic, versioned corpus of the Guru Granth Sahib.

Provide strict lexical counts (e.g., divine names, ritual terms).

Analyze structural features (register usage, co-occurrence patterns).

Provide bounded interpretive tagging (Nirgun/Sagun spectrum, etc.).

Publish auditable outputs consumable by a web application.
...
Run corpus validator before submitting changes.

Do not modify canonical Gurmukhi text.

Include tests for lexicon additions.

Document schema changes in docs/migrations/.
